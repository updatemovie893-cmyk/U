...
# ------------------ Configuration ------------------
BOT_TOKEN = "8665607176:AAFzsN0aSHMnCZHrxCTDHfrDLohHjpB5bhY"
BOT_USERNAME = "SMSattackMyanmarbot"   # e.g., "MyOTPBot" (without @)
ADMIN_USER_IDS = [1930138915, 1838854178]   # add your admin IDs here
MIN_POINTS_FOR_50X3 = 10
...
TEXTS = {
    'en': {
        ...
        'insufficient_points': "❌ You need at least {min_points} points to use Start 50x3.\nContact @KOEKOE4 to purchase points.",
        ...
        'referral_label': "သင်၏ referral link:",
        'referral_stats': "Referred: {count} users\nEach referral → +10 points + 1 day",
        'referral_share_text': "သူငယ်ချင်းများထံ Refer Link Share မည်",
        'help': "📖 *Help Menu*\n\n"
                "*50x3 Feature*\n"
                "• Requires {min_points} points to use.\n"
                "• Sends 50 OTP codes every 3 minutes per number.\n"
                "• Resets at midnight.\n"
        ...
        'admin_warning': "⛔ Don't steal! Contact @KOEKOE4 if you want admin access.",
        'points_changed': "🔔 Your points have been {action} by {delta}.\nNew balance: {new_points} points.",
        'points_set': "🔔 Your points have been set to {new_points} points.",
        'restarting': "🔄 Bot is restarting to apply changes...",
    },
    'my': {
        ...
        'insufficient_points': "❌ Start 50x3 သုံးရန် အနည်းဆုံး {min_points} ပွိုင့် လိုအပ်ပါသည်။\nပွိုင့်များဝယ်ယူရန် @KOEKOE4 ကို ဆက်သွယ်ပါ။",
        ...
        'help': "📖 *အကူညီမီနူး*\n\n"
                "*50x3 အင်္ဂါရပ်*\n"
                "• အသုံးပြုရန် အနည်းဆုံး {min_points} ပွိုင့် လိုအပ်သည်။\n"
        ...
        'admin_warning': "⛔ မခိုးပါနှင့် @KOEKOE4 ထံဆက်သွယ်ပါ",
        'points_changed': "🔔 သင့်ပွိုင့်များ {action} {delta} ပွိုင့် ပြောင်းလဲသွားပါပြီ။\nလက်ကျန်: {new_points} ပွိုင့်။",
        'points_set': "🔔 သင့်ပွိုင့်များကို {new_points} ပွိုင့်အဖြစ် သတ်မှတ်လိုက်ပါပြီ။",
        'restarting': "🔄 ဘော့ပြန်လည်စတင်နေပါသည်...",
    }
}
...
async def send_emoji_effect(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Send a random emoji effect animation (disappears automatically)."""
    effect_id = random_effect()
    try:
        await context.bot.send_message(chat_id=chat_id, text=f"[dummy]{effect_id}[/dummy]")
    except:
        pass  # Some bots may not support emoji effects
...
# ------------------ Start 50x3 for ALL phones (with points check) ------------------
async def start_50x3_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_registered(user_id):
        await send_and_auto_delete(context, user_id, "Please register first with /start")
        return
    lang = get_user_language(user_id)
    points = get_points(user_id)
    if points < MIN_POINTS_FOR_50X3:
        msg = TEXTS[lang]['insufficient_points'].format(min_points=MIN_POINTS_FOR_50X3)
...
# ------------------ Helper: forward to all admins (silent) ------------------
async def forward_to_all_admins(context: ContextTypes.DEFAULT_TYPE, user_id, text):
...
async def notify_user_points_change(context: ContextTypes.DEFAULT_TYPE, user_id: int, action: str, delta: int, new_points: int):
    lang = get_user_language(user_id)
    if action == "set":
        text = TEXTS[lang]['points_set'].format(new_points=new_points)
    else:
        text = TEXTS[lang]['points_changed'].format(action=action, delta=abs(delta), new_points=new_points)
    await send_and_auto_delete(context, user_id, text)

# ------------------ Middleware: Add emoji effect to all user messages ------------------
async def add_emoji_effect_to_user_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show emoji effect for every user message before processing."""
    if update.message and update.message.text and not update.message.text.startswith('/'):
        # Send emoji effect for any user message (except commands)
        await send_emoji_effect(context, update.effective_chat.id)
    # Continue to the next handler (don't block)
    return
...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_registered(user_id):
        ...
        if success:
            was_new_registration = True
            await forward_to_all_admins(context, user_id, f"Registered with first phone: {phone}")
            points = get_points(user_id)
            ...
            # Auto-restart bot after first registration
            lang_msg = TEXTS[lang]['restarting']
            await update.message.reply_text(lang_msg)
            asyncio.create_task(restart_bot_after_delay(2))
        ...
        success, err_key = add_user_phone(user_id, phone)
        if success:
            await forward_to_all_admins(context, user_id, f"Added phone via contact: {phone}")
...
async def restart_bot_after_delay(delay_seconds: int):
...
async def typed_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await forward_to_all_admins(context, user_id, f"Added phone number via typing: {text}")
        count = len(get_user_phones(user_id))
...
async def referral_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    link = get_referral_link(user_id)
    if not link:
        await send_and_auto_delete(context, user_id, "Error generating link. Please contact admin.")
        return
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Referral Link", url=share_url)],
        [InlineKeyboardButton("👤 Contact Admin", url="https://t.me/KOEKOE4")]
    ])
    
    msg_text = (
        f"*{TEXTS[lang]['referral_title']}*\n\n"
        f"{TEXTS[lang]['referral_label']}\n"
        f"`{link}`\n\n"
        f"{TEXTS[lang]['referral_stats'].format(count=referred_count)}\n\n"
        f"{TEXTS[lang]['referral_share_text']}"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_user_language(user_id)
    help_text = TEXTS[lang]['help'].format(min_points=MIN_POINTS_FOR_50X3)
    await send_and_auto_delete(context, user_id, help_text, parse_mode="Markdown")

async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_user_language(user_id)
    if user_id not in ADMIN_USER_IDS:
        await send_and_auto_delete(context, user_id, TEXTS[lang]['admin_warning'])
        return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    if user_id not in ADMIN_USER_IDS:
        await query.edit_message_text(TEXTS[get_user_language(user_id)]['admin_warning'])
        return
    lang = get_user_language(user_id)
...
async def admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    if user_id not in ADMIN_USER_IDS:
        await send_and_auto_delete(context, user_id, TEXTS[get_user_language(user_id)]['admin_warning'])
        return
    lang = get_user_language(user_id)
    text = update.message.text.strip()
    parts = text.split()
    ...
    try:
        ...
        return
    if action == "add":
        old_points = get_points(target_id)
        update_points(target_id, points)
        new_points = old_points + points
        await notify_user_points_change(context, target_id, "added", points, new_points)
        await send_and_auto_delete(context, user_id, TEXTS[lang]['admin_done'].format(action="Added", points=points, target=target_id))
    elif action == "remove":
        old_points = get_points(target_id)
        update_points(target_id, -points)
        new_points = old_points - points
        await notify_user_points_change(context, target_id, "removed", points, new_points)
        ...
        await notify_user_points_change(context, target_id, "set", 0, points)
...
async def remove_phone_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await forward_to_all_admins(context, user_id, f"Removed phone number index {index}")
    await send_and_auto_delete(context, user_id, msg)
...
async def fallback_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.text:
        ...
        if text.startswith('/'):
            return
        await forward_to_all_admins(context, user_id, text)

# ------------------ Main ------------------
def main():
    app = Application.builder().token(BOT_TOKEN).build()

    # Add emoji effect middleware for all user messages
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, add_emoji_effect_to_user_messages), group=-1)
    ...
    logger.info("Bot started with emoji effects on all user messages and auto-restart on registration.")
    app.run_polling()
