...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If user is not registered yet (first contact), register them
    if not is_registered(user_id):
        # But register_user was already called in start. Actually start already registers without phone.
        # So just add phone.
        ...
        if success:
            ...
            await update.message.reply_text(
                ...
                f"You now have {get_points(user_id)} points.\n"
                f"Referral link: {get_referral_link(user_id)}\n\n"
                f"You can add up to 4 more phone numbers using the '➕ Add Phone via Contact' button.\n"
                f"Tap *Start 50x3* to begin sending OTP codes.",
                reply_markup=main_keyboard(is_admin),
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(f"❌ {msg}", reply_markup=registration_keyboard())
    else:
        # Already registered – simply add this new phone number
        success, msg = add_user_phone(user_id, phone)
        await update.message.reply_text(msg, reply_markup=main_keyboard(user_id == ADMIN_USER_ID))
...
async def cancel_addition(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Addition cancelled.", reply_markup=main_keyboard(update.effective_user.id == ADMIN_USER_ID))
...
async def phone_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❌ Cancel":
        await update.message.reply_text("Cancelled.", reply_markup=main_keyboard(update.effective_user.id == ADMIN_USER_ID))
        return ConversationHandler.END
...
async def stop_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏹️ Stopping all active tasks...", reply_markup=main_keyboard(update.effective_user.id == ADMIN_USER_ID))
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Use '➕ Add Phone via Contact' to share a new contact.\n"
        ...
        reply_markup=main_keyboard(update.effective_user.id == ADMIN_USER_ID),
        parse_mode="Markdown"
    )
...
def main():
    ...
    # Conversation for phone selection (multiple numbers) and contact addition
    ...
    # We need to handle the text input after admin callback. Use a separate handler.
    ...
    logger.info("Bot started with contact sharing, referral inline link, admin panel, and 50x3 feature.")
    app.run_polling()
